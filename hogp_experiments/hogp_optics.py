...
def run_trial(
    ...
    train_Y = {k: c_batched(train_X_init, k) for k in train_X}
    train_Y_init = train_Y["rnd"]
    ...
    for i in range(n_batches):
        ...
        # last_x = normalize(train_X["ei"], bounds)[-1]
        # fixed_features = {4: last_x[0], 5: last_x[1], 6: last_x[2], 7: last_x[3]}
        fixed_features = None
        model_ei = fit_model(
        ...
        cands["ei"] = optimize_ei(
            qEI, bounds, **optimize_acqf_kwargs, fixed_features=fixed_features,
        )
        print(cands["ei"])
        del model_ei
        torch.cuda.empty_cache()
        ...
        last_x = normalize(train_X["ei_hogp_cf"], bounds)[-1]
        fixed_features = {4: last_x[0], 5: last_x[1], 6: last_x[2], 7: last_x[3]}
        ...
        cands["ei_hogp_cf"] = optimize_ei(
            qEI_hogp_cf, bounds, **optimize_acqf_kwargs, fixed_features=fixed_features,
        )
        del model_ei_hogp_cf
        torch.cuda.empty_cache()

        last_x = normalize(train_X["ei_hogp_cf_smooth"], bounds)[-1]
        fixed_features = {4: last_x[0], 5: last_x[1], 6: last_x[2], 7: last_x[3]}
        ...
        for k, Xold in train_X.items():
            ...
            if Xnew.shape[0] > 0:
                ...
                train_Y[k] = torch.cat([train_Y[k], c_batched(Xnew, k)])
    ...
    for k, v in suggested.items():
        if len(v) > 0:
            sugg_vals[k] = objective(c_batched(torch.stack(v), k)).tolist()
        else:
            sugg_vals[k] = v
