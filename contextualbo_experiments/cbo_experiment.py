...
def optimize_acqf_and_get_candidate(acq_func, bounds, batch_size, **kwargs):
    ...
    for step in range(n_batch):
        ...
        for key, model in models_dict.items():
            ...
            if not has_failed[key]:
                try:
                    print("starting training of ", key)
                    fit_model(model)
                    print("finished training of ", key)
            ...
            current_context = torch.randint(0, contexts, torch.Size())
            if model != "lcem":
                weights = torch.zeros(contexts, **tkwargs)
                weights[current_context] = 1.0
                objective = LinearMCObjective(weights)
                optim_kwargs = {}
            else:
                objective = None
                optim_kwargs = {
                    "fixed_feature": {init_x_lcem.shape[1]: current_context.item()}
                }
            ...
            if not has_failed[key]:
                try:
                    ...
                    new_x = optimize_acqf_and_get_candidate(
                        acqf, bounds, batch_size, **optim_kwargs
                    )
        ...
        logging.info(
            f"Step: {step} Best Achieved: {best_lcem} {best_lcemm} Last Times: {last_times[0]} {last_times[1]} "
        )
